...
def send(txt):
  Message(messages, nickname, txt).pack(fill=X, side=TOP)
  entry.delete(0, END)
...
#region create
crt_lbl1 = Label(frame2, text="Enter channel name:",
                 font=("Helvetica", 16)).inherit_bg()
crt_chn = Entry(frame2)
crt_lbl2 = Label(frame2, text="Enter nickname:",
                 font=("Helvetica", 16)).inherit_bg()
crt_nickname = Entry(frame2)
crt_btn = Button(frame2,
                 command=lambda: crt_proc(crt_chn.get(), crt_nickname.get()),
                 text="Create",
                 font=("Helvetica", 14))
#endregion
...
con_ip = Entry(frame2)
con_lbl2 = Label(frame2, text="Enter nickname:",
                 font=("Helvetica", 16)).inherit_bg()
con_nickname = Entry(frame2)
con_btn = Button(frame2,
                 command=lambda: con_proc(con_ip.get(), con_nickname.get()),
                 text="Connect",
                 font=("Helvetica", 14))
#endregion
...
btn = Button(enter_fr,
             ...
             width=20,
             bg="#4c9a15",
             activebackground="#60a231",
             borderwidth=0,
             command=lambda: send(entry.get()))
